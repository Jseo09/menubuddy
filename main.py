...
genai_chat = GoogleGenAIChatGenerator(model="gemini-2.0-flash-lite")  # Corrected model string
...
def clean_source_label(path):
    ...
    # If it's a local file path (image)
...
def ask_menu():
    ...
    try:
        res = safe_run(pipe.run, data={
            "prompt_builder": {"template_variables": {"documents": docs, "query": query}, "template": template}})
